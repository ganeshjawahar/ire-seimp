...
for line in lines:
	...
	if len(line)!=0:
		if line.startswith('I-SNE'):
			...
			split=line.split();
			res_bsne[0]=res_bsne[0]+float(split[1]);
			res_bsne[1]=res_bsne[1]+float(split[2]);
			res_bsne[2]=res_bsne[2]+float(split[3]);
...
for i in range(3):
	res_bsne[i]=(res_bsne[i]/numIterations);

print "# Iterations ="+str(numIterations);
print '\tP\tR\tF'
print 'B-SNE\t'+str(res_bsne[0])+'\t'+str(res_bsne[1])+'\t'+str(res_bsne[2]);
